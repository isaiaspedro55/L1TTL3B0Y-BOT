// ════════════════════════════════════════════════
// ✅ COMANDO !DARDOS — Toque no alvo para lançar o dardo.
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const DARDOS_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
.card{width:100%;max-width:440px;background:#fff;border:2px solid #e11d48;border-radius:22px;padding:13px;text-align:center;box-shadow:0 10px 30px #0002}
h1{font-size:30px;color:#e11d48;margin:2px}p{color:#64748b;font-size:14px;margin:3px 0 8px}
#placar{font-size:18px;font-weight:900;margin:7px;color:#e11d48}
...
.modal button{margin-top:14px;width:100%;background:#e11d48;color:#fff}
...
canvas{width:100%;max-height:68vh;border-radius:18px;border:2px solid #e11d48;touch-action:none;background:#f8fafc}
</style>
</head>
<body>
<div class="card">
<h1>🎯 Dardos</h1><p>Toque no alvo para lançar o dardo.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><canvas id="cv" width="360" height="360"></canvas></div>
<div id="msg">Toque em JOGAR para começar.</div>
<div class="row"><button onclick="resetar()" style="background:#e5e7eb">🔄 Novo</button><button onclick="som()" id="snd" style="background:#e11d48;color:#fff">🔊 Som</button></div>
</div>
<script>
...
function initGame(){let c=document.getElementById("cv"),x=c.getContext("2d");x.fillStyle="#fee2e2";x.fillRect(0,0,360,360);[110,75,40].forEach((r,i)=>{x.beginPath();x.fillStyle=i===0?"#111827":i===1?"#ef4444":"#fbbf24";x.arc(180,180,r,0,7);x.fill()});c.onclick=e=>{let r=c.getBoundingClientRect(),x1=(e.clientX-r.left)/r.width*360,y1=(e.clientY-r.top)/r.height*360,d=Math.hypot(x1-180,y1-180),p=d<40?10:d<75?5:d<110?2:0;add(p);msg(p?"🎯 Acertou! +"+p:"Fora!");beep(p?1000:220)} }
</script>
</body></html>`;

async function enviarDardos(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "dardos_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🎯 Dardos" }],
