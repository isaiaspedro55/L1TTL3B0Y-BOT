// ════════════════════════════════════════════════
// ✅ COMANDO !BILHAR — Toque na mesa para fazer uma tacada.
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const BILHAR_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
<body>
<div class="card">
<h1>🎱 Bilhar</h1><p>Toque na mesa para fazer uma tacada.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><canvas id="cv" width="360" height="460"></canvas></div>
...
function initGame(){let c=document.getElementById("cv"),x=c.getContext("2d");x.fillStyle="#075e3b";x.fillRect(0,0,360,460);x.font="45px sans-serif";x.fillText("⚪",80,240);x.fillText("🎱",270,240);c.onclick=()=>{add(2);msg("🎱 Tacada!");beep(750)}}
</script>
</body></html>`;

async function enviarBilhar(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "bilhar_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🎱 Bilhar" }],
