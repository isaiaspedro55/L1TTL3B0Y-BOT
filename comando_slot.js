// ════════════════════════════════════════════════
// ✅ COMANDO !SLOT — Alinhe 3 símbolos iguais.
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const SLOT_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
<body>
<div class="card">
<h1>🎰 Slot</h1><p>Alinhe 3 símbolos iguais.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><div style="width:100%"><div id="reels" class="grid" style="grid-template-columns:repeat(3,1fr);background:#111827;padding:10px;border-radius:18px"><button class="cell" id="r0">🍒</button><button class="cell" id="r1">⭐</button><button class="cell" id="r2">💎</button></div><button class="primary" style="width:100%;margin-top:12px" onclick="girar()">🎰 GIRAR</button></div></div>
...
let busy=false,sym=["🍒","🍋","⭐","💎","🔔","7️⃣"];
function initGame(){}
function girar(){if(busy)return;busy=true;let n=0,t=setInterval(()=>{for(let i=0;i<3;i++)document.getElementById("r"+i).textContent=sym[Math.floor(Math.random()*sym.length)];beep(350+n*20,.03);if(++n>14){clearInterval(t);let a=[0,1,2].map(i=>document.getElementById("r"+i).textContent),u=new Set(a).size;if(u===1){add(10);msg("💎 JACKPOT! +10");beep(1100,.2)}else if(u===2){add(3);msg("🔥 Dupla! +3")}else msg("😅 Tente novamente");busy=false}},70)}
</script>
</body></html>`;

async function enviarSlot(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "slot_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🎰 Slot" }],
