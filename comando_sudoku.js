// ════════════════════════════════════════════════
// ✅ COMANDO !SUDOKU — Complete o tabuleiro 4×4.
// ════════════════════════════════════════════════
...
<body>
<div class="card">
<h1>🔢 Sudoku</h1><p>Complete o tabuleiro 4×4.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><div id="sudoku" class="grid" style="grid-template-columns:repeat(4,1fr);max-width:350px"></div></div>
...
let sol=[1,2,3,4,3,4,1,2,2,1,4,3,4,3,2,1],given=[1,0,3,0,0,4,0,2,2,0,4,0,0,3,0,1],vals=[];function initGame(){vals=[...given];render()}function render(){document.getElementById("sudoku").innerHTML=sol.map((s,i)=>`<button class="cell" ${given[i]?"disabled":""} onclick="cell(${i})">${vals[i]||"?"}</button>`).join("")}function cell(i){vals[i]=vals[i]?vals[i]%4+1:1;add(1);beep(650);render();if(vals.every((v,i)=>v===sol[i]))msg("🏆 Sudoku completo!")}
</script>
</body></html>`;

async function enviarSudoku(sock, jid, quotedMsg) {
  const htmlPayload = {
