// ════════════════════════════════════════════════
// ✅ COMANDO !FORCA — Descubra a palavra antes de ficar sem tentativas.
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const FORCA_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
<body>
<div class="card">
<h1>🔤 Forca</h1><p>Descubra a palavra antes de ficar sem tentativas.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><div style="width:100%"><h2 id="word">_ _ _ _ _</h2><p id="tries">Tentativas: 6</p><input id="letter" maxlength="1" placeholder="Digite uma letra"><button class="primary" onclick="guess()" style="width:100%;margin-top:8px">TENTAR</button></div></div>
...
let word="",shown=[],tries=6;function initGame(){word=["JAVASCRIPT","ANDROID","WHATSAPP","MUSICA","PROGRAMAR"][Math.floor(Math.random()*5)];shown=Array(word.length).fill("_");tries=6;upd()}function upd(){document.getElementById("word").textContent=shown.join(" ");document.getElementById("tries").textContent="Tentativas: "+tries}function guess(){let l=document.getElementById("letter").value.toUpperCase();if(!l)return;if(word.includes(l)){[...word].forEach((c,i)=>{if(c===l)shown[i]=l});add(1);beep(850)}else{tries--;beep(220)}document.getElementById("letter").value="";upd();if(!shown.includes("_"))msg("🏆 Palavra descoberta!");else if(!tries)msg("💀 Era "+word)}
</script>
</body></html>`;

async function enviarForca(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "forca_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🔤 Forca" }],
