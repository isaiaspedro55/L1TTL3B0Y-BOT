// ════════════════════════════════════════════════
// ✅ COMANDO !CAÇA-BUGS — Toque nos bugs antes que mudem de lugar.
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const CACABUGS_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
<body>
<div class="card">
<h1>🪲 Caça-Bugs</h1><p>Toque nos bugs antes que mudem de lugar.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><div id="bugs" style="width:100%;height:420px;position:relative;background:#dcfce7;border-radius:18px"></div></div>
...
function initGame(){moveBug()}function moveBug(){let a=document.getElementById("bugs");a.innerHTML="";let b=document.createElement("button");b.textContent="🪲";b.style.cssText="position:absolute;left:"+Math.random()*80+"%;top:"+Math.random()*80+"%;font-size:32px;background:none";b.onclick=()=>{add(1);beep(1000);moveBug()};a.appendChild(b);setTimeout(moveBug,900)}
</script>
</body></html>`;

async function enviarCacaBugs(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "cacabugs_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
          __typename: "GenAIaeacdsnwHtmlPrimitive",
          payload: CACABUGS_HTML,
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🪲 Caça-Bugs" }],
