// ════════════════════════════════════════════════
// ✅ COMANDO !LABIRINTO — Encontre a saída
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const LABIRINTO_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
body{display:flex;justify-content:center;background:#faf5ff;font-family:system-ui;padding:7px;min-height:100vh;color:#172033}
.card{width:100%;max-width:440px;background:#fff;border:2px solid #9333ea;border-radius:22px;padding:13px;text-align:center;box-shadow:0 10px 30px #0002}
h1{font-size:30px;color:#9333ea;margin:2px}p{color:#64748b;font-size:14px;margin:3px 0 8px}
#placar{font-size:18px;font-weight:900;margin:7px;color:#9333ea}
...
.modal button{margin-top:14px;width:100%;background:#9333ea;color:#fff}
...
canvas{width:100%;max-height:68vh;border-radius:18px;border:2px solid #9333ea;touch-action:none;background:#f8fafc}
</style>
</head>
<body>
<div class="card">
<h1>🧩 Labirinto</h1><p>Encontre a saída</p>
<div id="placar">Pontos: 0</div>
<div id="area"><div style="width:100%;text-align:center"><canvas id="cv" width="360" height="420"></canvas><div class="row"><button class="primary" onclick="act(-1)">⬅️</button><button class="primary" onclick="act(0)">🎮</button><button class="primary" onclick="act(1)">➡️</button></div></div></div>
<div id="msg">Toque em JOGAR para começar.</div>
<div class="row"><button onclick="resetar()" style="background:#e5e7eb">🔄 Novo</button><button onclick="som()" id="snd" style="background:#9333ea;color:#fff">🔊 Som</button></div>
</div>
<script>
...
let ctx,n=0;function initGame(){ctx=document.getElementById("cv").getContext("2d");draw()}function act(d){n++;add(1);beep(600+n*20);draw();if(n>=10)msg("🏆 Desafio concluído!")}function draw(){ctx.fillStyle="#0f172a";ctx.fillRect(0,0,360,420);ctx.font="48px sans-serif";ctx.fillText("🧩",60+(n*23)%250,220)}
</script>
</body></html>`;

async function enviarLabirinto(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "labirinto_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
          __typename: "GenAIaeacdsnwHtmlPrimitive",
          payload: LABIRINTO_HTML,
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🧩 Labirinto" }],
...
module.exports = { enviarLabirinto };
