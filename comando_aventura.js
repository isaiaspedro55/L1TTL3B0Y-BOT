// ════════════════════════════════════════════════
// ✅ COMANDO !AVENTURA — Escolha seu caminho e avance na história.
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const AVENTURA_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no">
<title>🗺️ Aventura</title>
<style>
...
.card{width:100%;max-width:440px;background:#fff;border:2px solid #8b5cf6;border-radius:22px;padding:13px;text-align:center;box-shadow:0 10px 30px #0002}
h1{font-size:30px;color:#8b5cf6;margin:2px}p{color:#64748b;font-size:14px;margin:3px 0 8px}
#placar{font-size:18px;font-weight:900;margin:7px;color:#8b5cf6}
...
.modal button{margin-top:14px;width:100%;background:#8b5cf6;color:#fff}
...
canvas{width:100%;max-height:68vh;border-radius:18px;border:2px solid #8b5cf6;touch-action:none;background:#f8fafc}
</style>
</head>
<body>
<div class="card">
<h1>🗺️ Aventura</h1><p>Escolha seu caminho e avance na história.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><div style="width:100%" id="story"><h2>🌲 Você chegou a uma floresta misteriosa.</h2><div class="grid" style="grid-template-columns:1fr 1fr;margin-top:15px"><button class="primary" onclick="choice(1)">🌲 Floresta</button><button class="primary" onclick="choice(2)">🏰 Castelo</button></div></div></div>
<div id="msg">Toque em JOGAR para começar.</div>
<div class="row"><button onclick="resetar()" style="background:#e5e7eb">🔄 Novo</button><button onclick="som()" id="snd" style="background:#8b5cf6;color:#fff">🔊 Som</button></div>
</div>
<script>
...
let step=0;function initGame(){step=0}function choice(n){step++;add(2);beep(800);let t=["✨ Você encontrou uma ponte.","🗝️ Você achou uma chave.","🏆 Você encontrou o tesouro!"];document.getElementById("story").innerHTML="<h2>"+t[Math.min(step-1,2)]+"</h2><button class='primary' onclick='choice(1)'>➡️ Continuar</button>";if(step>=3)msg("🏆 Aventura concluída!")}
</script>
</body></html>`;

async function enviarAventura(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "aventura_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
          __typename: "GenAIaeacdsnwHtmlPrimitive",
          payload: AVENTURA_HTML,
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🗺️ Aventura" }],
