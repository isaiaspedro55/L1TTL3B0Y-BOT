// ════════════════════════════════════════════════
// ✅ COMANDO !QUEBRA-BLOCOS — Quebre os blocos
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const QUEBRABLOCOS_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
.card{width:100%;max-width:440px;background:#fff;border:2px solid #c2410c;border-radius:22px;padding:13px;text-align:center;box-shadow:0 10px 30px #0002}
h1{font-size:30px;color:#c2410c;margin:2px}p{color:#64748b;font-size:14px;margin:3px 0 8px}
#placar{font-size:18px;font-weight:900;margin:7px;color:#c2410c}
...
.modal button{margin-top:14px;width:100%;background:#c2410c;color:#fff}
...
canvas{width:100%;max-height:68vh;border-radius:18px;border:2px solid #c2410c;touch-action:none;background:#f8fafc}
</style>
</head>
<body>
<div class="card">
<h1>🧱 Quebra-Blocos</h1><p>Quebre os blocos</p>
...
<div class="row"><button onclick="resetar()" style="background:#e5e7eb">🔄 Novo</button><button onclick="som()" id="snd" style="background:#c2410c;color:#fff">🔊 Som</button></div>
</div>
<script>
...
async function enviarQuebraBlocos(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "quebrablocos_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
          __typename: "GenAIaeacdsnwHtmlPrimitive",
          payload: QUEBRABLOCOS_HTML,
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "🧱 Quebra-Blocos" }],
...
module.exports = { enviarQuebraBlocos };
