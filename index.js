...
// ═══════════════════════════════════════════════════════
//  ✅ MENU — Carousel com listMessage fallback
// ═══════════════════════════════════════════════════════

function buildSecoes(isDono){
  const secoes=[
    {title:"🎵 DOWNLOADS",rows:[
      {header:"🎵 Música & Vídeo",title:"play, mp3, mp4, mp4hd, sticker...",id:"cat_musica"},
      {header:"📱 Redes Sociais",title:"TikTok, Instagram, Twitter, Spotify...",id:"cat_social"},
    ]},
    {title:"🧠 FERRAMENTAS",rows:[
      {header:"🧠 Inteligência IA",title:"IA, traduzir, analisar imagens...",id:"cat_ia"},
      {header:"🎮 Jogos",title:"Quiz, Forca, V/F, Caça-palavras...",id:"cat_jogos"},
      {header:"🔢 Utilidades",title:"QR, Calc, Placar, Tempo, ToURL...",id:"cat_util"},
    ]},
    {title:"⚙️ EXTRAS",rows:[
      {header:"🕵️ Extras",title:"Shazam ⚡, Editor IA, View-Once...",id:"cat_extra"},
      {header:"📁 Arquivos & VPN",title:"Ficheiros .ehi .ovpn...",id:"cat_arq"},
      {header:"🛡️ Administração",title:"Ban, Silenciar, Fechar, ScanLink...",id:"cat_adm"},
    ]}
  ];
  if(isDono){
    secoes.push({title:"👑 DONO",rows:[
      {header:"🏘️ Gestão de Grupos",title:"ChaTon, SMS, GSMS...",id:"cat_grup"},
      {header:"👑 Área do Dono",title:"Ergue-se, !setfoto, Senha...",id:"cat_dono"},
    ]});
  }
  return secoes;
}
...
｜✦ 🕐 HORA: *${hora}*`;
  ...
  try{
    if(botFotoBuffer) await sock.sendMessage(jid,{image:botFotoBuffer,caption:textoMenu},{quoted:msg});
    else if(ppBotUrl)  await sock.sendMessage(jid,{image:{url:ppBotUrl},caption:textoMenu},{quoted:msg});
    else               await sock.sendMessage(jid,{text:textoMenu},{quoted:msg});
    await new Promise(r=>setTimeout(r,700));
    await sock.sendMessage(jid,{
      listMessage:{title:"🌀 *LORDE LÁ DJUM v3.5*",description:"Selecciona uma categoria:",footerText:"© LORDE LÁ DJUM v3.5",buttonText:"≡ MENU",listType:1,sections:secoes}
    });
    return;
  }catch(e){console.log("⚠️ listMessage:",e.message);}

  // ✅ Fallback final: texto + numerado
  try{await sock.sendMessage(jid,{text:textoMenu},{quoted:msg});}catch{}
...
◎ ─ *${P}shazam* ⚡⚡⚡
    ...
    _↳ reconhece música com animação_
│
◎ ─ *${P}busca*
    _↩️ responde nota de voz_
    _↳ reconhece música sem animação_
│
◎ ─ *${P}editar* [instrução]
...
// ═══════════════════════════════════════════════════════
//  ✅ UPLOADS — catbox fallback para "Media upload failed"
// ═══════════════════════════════════════════════════════
...
async function uploadParaCatbox(buffer,nome,mimeType){
  ...
  const{data}=await axios.post("https://catbox.moe/user/api.php",formData,{
    headers:{...formData.getHeaders()},timeout:180000,httpsAgent,
...
async function uploadParaTelegraph(buffer){
  const formData=new FormData(); let mimeType="image/jpeg",ext="jpg";
  if(buffer[0]===0x89&&buffer[1]===0x50){mimeType="image/png";ext="png";}
  formData.append("file",buffer,{filename:`img.${ext}`,contentType:mimeType});
  const{data}=await axios.post("https://telegra.ph/upload",formData,{headers:{...formData.getHeaders()},timeout:30000,httpsAgent});
  if(data?.[0]?.src) return `https://telegra.ph${data[0].src}`;
  throw new Error("Telegraph falhou");
}

// ✅ ENVIAR ÁUDIO — catbox fallback
async function enviarAudio(sock,jid,filePath,msgCitada){
  ...
  try{await sock.sendMessage(jid,{audio:buf,mimetype:mime,ptt:false},msgCitada?{quoted:msgCitada}:{}); cleanup(); console.log("✅ Áudio (buffer)"); return;}catch(e){console.log("⚠️ áudio buffer:",e.message);}
  try{
    console.log("⬆️ Upload áudio para catbox...");
    ...
    cleanup(); console.log("✅ Áudio (catbox)"); return;
  ...
  if(converteu){
    try{
      ...
      cleanup(); console.log("✅ Áudio (catbox raw)"); return;
  ...
  }catch(e){cleanup(); throw new Error("Falhou ao enviar áudio");}
}

// ✅ ENVIAR VÍDEO — catbox fallback
async function enviarVideo(sock,jid,filePath,caption,mentions,msgCitada){
  ...
  try{await sock.sendMessage(jid,{video:buf,caption,mentions},msgCitada?{quoted:msgCitada}:{}); console.log("✅ Vídeo (buffer)"); return;}catch(e){console.log("⚠️ vídeo buffer:",e.message);}
  try{
    console.log("⬆️ Upload vídeo para catbox...");
    ...
    console.log("✅ Vídeo (catbox)"); return;
...
async function executarReconhecimentoMusica(sock,jid,msg,sender,comAnimacao){
  ...
  if(!audioData){
    await enviarSemFoto(sock,jid,`┌─⊱ 『 ${comAnimacao?"⚡ SHAZAM":"🎵 BUSCA"} 』 ⊰─┐\n│\n◎ ─ *${CONFIG.PREFIXO}${comAnimacao?"shazam":"busca"}*\n    _↩️ responde nota de voz_\n    _↳ reconhece a música_\n│\n└──────────────────────────────⊰`);
    return;
  }
  if(comAnimacao){
    await reagir(sock,msg,"⚡");
  ...
  }catch(e){await reagir(sock,msg,"❌"); await enviarSemFoto(sock,jid,`❌ Erro: ${e.message}`);}
}
  ...
  try{
    ...
    sock.ev.on("messages.upsert",async({messages,type})=>{
      try{
        ...
        // ✅ HANDLER: listResponseMessage (clique em categoria)
        ...
        // ✅ HANDLER: interactiveResponseMessage (Carousel)
        ...
        // !ergue-se — ÚNICA mensagem automática
...
│
└──────────────────────────────⊰
_Após pagamento, envia comprovativo!_ 🧾`,ppBotUrl,msg);
          await reagir(sock,msg,"💰"); return;
        }
        ...
        if(comando==="addai"){
          ...
          try{
            ...
            await reagir(sock,msg,"✅");
          }catch(e){await enviarSemFoto(sock,jid,`❌ Não foi possível adicionar a Meta AI.\n_${e.message.slice(0,80)}_`); await reagir(sock,msg,"❌");}
          return;
        }
