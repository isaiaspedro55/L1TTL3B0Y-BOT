...
const MENU_NUMEROS = {
  "1":"cat_musica","2":"cat_social","3":"cat_ia","4":"cat_jogos",
  "5":"cat_util","6":"cat_extra","7":"cat_arq","8":"cat_adm",
  "9":"cat_grup","0":"cat_dono",
};

const ARQ_EXTS=[".ehi",".npv",".hia",".ovpn",".conf",".vpn",".key",".cert",".p12",".vless",".vmess"];

function getTipoMsg(msg){const m=msg?.message; if(!m) return "📄 Outro"; if(m.conversation||m.extendedTextMessage) return "💬 Texto"; if(m.imageMessage) return "🖼️ Imagem"; if(m.videoMessage) return "🎥 Vídeo"; if(m.audioMessage||m.pttMessage) return "🎙️ Áudio"; if(m.stickerMessage) return "🎭 Figurinha"; if(m.documentMessage) return "📄 Documento"; if(m.viewOnceMessage||m.viewOnceMessageV2||m.viewOnceMessageV2Extension) return "👁️ Visual única"; return "📄 Outro";}
...
const VOF_BANCO=[{p:"O sol é uma estrela.",r:"verdadeiro"},{p:"A baleia é um peixe.",r:"falso"},{p:"O coração tem 4 câmaras.",r:"verdadeiro"},{p:"Angola tem 18 províncias.",r:"verdadeiro"},{p:"A água ferve a 50°C.",r:"falso"},{p:"O elefante é o maior animal terrestre.",r:"verdadeiro"},{p:"A Lua tem atmosfera.",r:"falso"},{p:"O tubarão é um mamífero.",r:"falso"},{p:"Luanda é capital de Angola.",r:"verdadeiro"},{p:"O diamante é o mineral mais duro.",r:"verdadeiro"},{p:"O Brasil tem mais de 200 milhões de habitantes.",r:"verdadeiro"},{p:"O ouro é um metal.",r:"verdadeiro"},{p:"A África é o maior continente do mundo.",r:"falso"},{p:"O golfinho é um mamífero.",r:"verdadeiro"},{p:"A Lua é maior que a Terra.",r:"falso"},{p:"O Python é uma linguagem de programação.",r:"verdadeiro"},{p:"O caracol tem concha.",r:"verdadeiro"},{p:"O Rio Nilo fica na Ásia.",r:"falso"}];
...
const PERFIS_ZOADA=["😂 Deus criou esta pessoa e perguntou: 'O que fiz?!' 💀","🤣 A face assusta os espelhos! 💀","😭 Esta pessoa chegou e o WiFi ficou lento! 🚶🏿‍♂️","💀 Dizem que já foi bonita... antes da câmara frontal! 📸😂","🤡 Acorda às 6h, olha pro espelho e volta a dormir! 😂"];
...
// ═══════════════════════════════════════════════════════
//  ✅ MENU COM BOTÕES REAIS (interactiveMessage nativeFlow)
// ═══════════════════════════════════════════════════════
...
｜✦ 🕐 HORA: *${hora}*`;
  ...
  // ✅ Tenta enviar com botões reais (interactiveMessage)
  let botaoFuncionou = false;
  try {
    let imgBuf = null;
    if (ppBotUrl) {
      try {
        const resp = await axios.get(ppBotUrl,{responseType:"arraybuffer",timeout:8000,httpsAgent});
    ...
    const botoesMenu = [
      {
        name: "quick_reply",
        buttonParamsJson: JSON.stringify({ display_text: "≡  MENU", id: "btn_abrir_menu" })
      },
      {
        name: "quick_reply",
        buttonParamsJson: JSON.stringify({ display_text: `👑 ${CONFIG.DONO_NOME}`, id: "btn_dono" })
      }
    ];

    if (imgBuf) {
      // Com imagem no header
      const interMsg = generateWAMessageFromContent(jid, {
        viewOnceMessage: {
          message: {
            interactiveMessage: proto.Message.InteractiveMessage.create({
              header: proto.Message.InteractiveMessage.Header.create({
                ...
                imageMessage: proto.Message.ImageMessage.create({
                  url: ppBotUrl,
                  mimetype: "image/jpeg",
                  jpegThumbnail: imgBuf.slice(0, 3000),
                })
              }),
              body: proto.Message.InteractiveMessage.Body.create({ text: textoMenu }),
              footer: proto.Message.InteractiveMessage.Footer.create({ text: "© LORDE LÁ DJUM v3.5" }),
              nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({ buttons: botoesMenu })
            })
          }
        }
      }, { quoted: msg });

      await sock.relayMessage(jid, interMsg.message, { messageId: interMsg.key.id });
      botaoFuncionou = true;
    } else {
      // Sem imagem
      const interMsg = generateWAMessageFromContent(jid, {
    ...
    console.log("⚠️ interactiveMessage falhou:", e.message);
  }

  // Fallback: imagem + menu numerado
  if (!botaoFuncionou) {
    try {
      if (ppBotUrl) await sock.sendMessage(jid,{image:{url:ppBotUrl},caption:textoMenu},{quoted:msg});
      else await sock.sendMessage(jid,{text:textoMenu},{quoted:msg});
    } catch { await sock.sendMessage(jid,{text:textoMenu},{quoted:msg}); }
...
│
◎ ─ *${P}play* [música]
    _↳ baixa e envia o áudio_
◎ ─ *${P}mp3* [música] → _HD_
    _↳ qualidade máxima_
◎ ─ *${P}mp4* [nome/link] → _480p_
    _↳ vídeo normal_
◎ ─ *${P}mp4hd* [nome/link] → _720p_
    _↳ vídeo alta definição_
│
◎ ─ *${P}mostre* [pesquisa]
◎ ─ *${P}foto* [url]
◎ ─ *${P}doc* [url]
...
◎ ─ *${CONFIG.PREFIXO}set* [senha] → _troca senha_
...
// ═══════════════════════════════════════════════════════
//  ✅ BUSCA INFO MÚSICA antes de baixar (como na imagem)
// ═══════════════════════════════════════════════════════
...
async function buscarInfoMusica(entrada) {
  ...
  try {
    const json = await runCmd(
      `yt-dlp --dump-json --no-playlist --no-warnings --force-ipv4 --geo-bypass ` +
      `--extractor-args "youtube:player_client=android,ios" ` +
      `--add-header "User-Agent:${UA}" "${pesquisa}" 2>/dev/null | head -1`
    );
    if (!json || !json.trim()) return null;
    const info = JSON.parse(json.trim());
    return {
      titulo:   info.title        || "N/A",
      artista:  info.uploader     || info.channel || "N/A",
      duracao:  formatarDuracao(info.duration || 0),
      thumb:    info.thumbnail    || null,
      url:      info.webpage_url  || entrada,
    };
  } catch { return null; }
}

// ✅ Envia card de info da música (estilo da imagem)
...
• 🤖 | BOT → 🌀LORDE-DJUM v3.5
• 👤 | USUÁRIO → ${sender.split("@")[0].split(":")[0]}
• 🎵 | TÍTULO → ${info.titulo}
• 📺 | CANAL → ${info.artista}
...
• 🔗 | LINK → ${info.url}`;
  ...
  try {
    if (info.thumb) {
      await sock.sendMessage(jid, { image: { url: info.thumb }, caption }, { quoted: msg });
    } else {
  ...
  try{
    ...
    for(const [gJid,meta] of Object.entries(grupos)){
      try{
        ...
        if(donoNoGrupo){
          gruposAtivados.add(gJid); activados++;
          if(ehPrimeira){
  ...
  try{
    ...
    sock.ev.on("messages.upsert",async({messages,type})=>{
      try{
        ...
        // ✅ HANDLER: interactiveResponseMessage (botões nativeFlow)
        const interResp = msg.message?.interactiveResponseMessage;
        if(interResp){
          let btnId = null;
          try{
            const nf = interResp.nativeFlowResponseMessage;
            if(nf?.paramsJson){
              const params = JSON.parse(nf.paramsJson);
              btnId = params.id || params.selectedId || null;
            }
          }catch{}
          if(!btnId && interResp.body) btnId = interResp.body;
          if(btnId){
            ...
            if(!isDono&&!senhasAprovadas.has(sender)){
              let iA=false; if(isGrupo){try{const meta=await sock.groupMetadata(jid),admins=meta.participants.filter(p=>p.admin).map(p=>extrairJid(p.id||p)); iA=admins.includes(sender);}catch{} if(iA){senhasAprovadas.add(sender);}else return;}else return;
            }
        ...
        // ✅ HANDLER: buttonsResponseMessage (fallback)
        ...
        if(!texto.startsWith(CONFIG.PREFIXO)){
          ...
          if(/^[0-9]$/.test(texto.trim())){
            ...
            if(estadoMenu&&(Date.now()-estadoMenu.timestamp)<120000){
              ...
              if(catId){
                if((catId==="cat_grup"||catId==="cat_dono")&&!estadoMenu.isDono){
                  await enviarSemFoto(sock,jid,`🔒 _Apenas o dono tem acesso._`); return;
                }
...
_© LORDE LÁ DJUM v3.5 — 24/7_ 🟢`,ppBotUrl); return;}
        ...
        // ✅ PLAY — com card de info (estilo da imagem)
        if(comando==="play"&&args.length>0){
          const entrada=args.join(" ");
          await enviarSemFoto(sock,jid,`🔍 A procurar: _${entrada}_\n⏳`);
          // Busca info primeiro
          const info=await buscarInfoMusica(entrada);
          if(info){
            await enviarCardMusica(sock,jid,msg,info,sender,"play");
            await new Promise(r=>setTimeout(r,500));
          }
          await enviarSemFoto(sock,jid,`⬇️ A baixar áudio...\n⏳`);
          let arqFinal=null;
          try{arqFinal=await downloadMusica(entrada,false);}catch{}
          if(!arqFinal||!fs.existsSync(arqFinal)){await enviarSemFoto(sock,jid,`❌ Não encontrei.`); await reagir(sock,msg,"❌"); return;}
          try{await enviarAudio(sock,jid,arqFinal,msg); await reagir(sock,msg,"✅"); addXP(sender,5);}catch{await enviarSemFoto(sock,jid,`❌ Erro.`);}
          setTimeout(()=>{try{fs.removeSync(arqFinal);}catch{}},15000);
          return;
        }

        // ✅ MP3 HD — com card de info
        if(comando==="mp3"&&args.length>0){
          const entrada=args.join(" ");
          await enviarSemFoto(sock,jid,`🔍 A procurar: _${entrada}_\n⏳`);
          const info=await buscarInfoMusica(entrada);
          if(info){
            await enviarCardMusica(sock,jid,msg,info,sender,"mp3");
            await new Promise(r=>setTimeout(r,500));
          }
          await enviarSemFoto(sock,jid,`⬇️ A baixar MP3 HD...\n⏳`);
          let arqFinal=null;
          try{arqFinal=await downloadMusica(entrada,true);}catch{}
          if(!arqFinal||!fs.existsSync(arqFinal)){await enviarSemFoto(sock,jid,`❌ Não encontrei.`); await reagir(sock,msg,"❌"); return;}
          try{await enviarAudio(sock,jid,arqFinal,msg); await reagir(sock,msg,"✅"); addXP(sender,5);}catch{await enviarSemFoto(sock,jid,`❌ Erro.`);}
          setTimeout(()=>{try{fs.removeSync(arqFinal);}catch{}},15000);
          return;
        ...
        if(comando==="busca"||comando==="shazam"){
          ...
          if(!audioData){await enviarSemFoto(sock,jid,`┌─⊱ 『 ⚡ SHAZAM 』 ⊰─┐\n│\n◎ ─ *${CONFIG.PREFIXO}busca*\n    _↩️ responde nota de voz_\n    _↳ reconhece qualquer música_\n│\n└──────────────────────────────⊰`); return;}
          await reagir(sock,msg,"⚡");
          ...
          try{
            ...
            if(resultado.status==="success"&&resultado.result){
              ...
              await reagir(sock,msg,"🎵"); addXP(sender,5);
            }else{await reagir(sock,msg,"❌"); await enviarSemFoto(sock,jid,`┌─⊱ 『 ⚡ SHAZAM 』 ⊰─┐\n│\n◎ ─ ❌ Música não reconhecida.\n◎ ─ _Tenta com áudio mais claro._\n│\n└──────────────────────────────⊰`);}
          }catch(e){await reagir(sock,msg,"❌"); await enviarSemFoto(sock,jid,`❌ Erro no Shazam: ${e.message}`);}
          return;
        }
        ...
        if(comando==="editar"){
          ...
          try{
            ...
            await reagir(sock,msg,"✅"); addXP(sender,5);
          }catch(e){console.log("❌ editar:",e.message); await enviarSemFoto(sock,jid,`❌ Erro: ${e.message.slice(0,100)}`); await reagir(sock,msg,"❌");}
          return;
        }
        ...
        if(comando==="scanlink"){
          ...
          if(!historico.length){await enviarComFoto(sock,jid,`┌─⊱ 『 🔍 SCAN 』 ⊰─┐\n│\n◎ ─ 📭 Sem histórico ainda.\n│\n└──────────────────────────────⊰`,ppBotUrl); return;}
        ...
        if(comando==="sms"){
          ...
          try{const grupos=await sock.groupFetchAllParticipating(); const nomeGrupo=grupos[grupoJid]?.subject||"Grupo"; const meta=await sock.groupMetadata(grupoJid); const membros=meta.participants.map(p=>extrairJid(p.id||p)); await enviarSemFoto(sock,jid,`📤 A enviar para *${membros.length}* membros...\n⏳`); let enviados=0,erros=0; for(const membro of membros){if(ehDono(membro)) continue; try{await sock.sendMessage(membro,{text:`📢 *Mensagem Privada*\n✦ ─────────── ✦\n\n${mensagem}\n\n✦ ─────────── ✦\n_Enviado por: ${CONFIG.DONO_NOME}_\n_Grupo: ${nomeGrupo}_`}); enviados++; await new Promise(r=>setTimeout(r,600));}catch{erros++;}} await enviarComFoto(sock,jid,`┌─⊱ 『 ✅ SMS ENVIADA! 』 ⊰─┐\n│\n◎ ─ 📊 Enviada: *${enviados}*\n◎ ─ ❌ Erros: *${erros}*\n◎ ─ 👥 Grupo: *${nomeGrupo}*\n│\n└──────────────────────────────⊰`,ppBotUrl); await reagir(sock,msg,"📢");}catch(e){await enviarSemFoto(sock,jid,`❌ ${e.message}`);}
          return;
        }
        ...
        if(comando==="arqdelete"){
          const fichs=fs.readdirSync("./vpn");
          if(!args[0]){await enviarComFoto(sock,jid,`┌─⊱ 『 🗑️ ELIMINAR 』 ⊰─┐\n│\n◎ ─ *${CONFIG.PREFIXO}arqdelete* [nome ou nº]\n│\n${fichs.length>0?fichs.map((f,i)=>`◎ ─ ${i+1}. ${f}`).join("\n"):"◎ ─ _Sem ficheiros_"}\n│\n└──────────────────────────────⊰`,ppBotUrl); return;}
