// ════════════════════════════════════════════════
// ✅ COMANDO !DIGITAÇÃO — Digite a frase exatamente como aparece.
// ════════════════════════════════════════════════
const { generateWAMessageFromContent } = require("@itsliaaa/baileys");

const DIGITACAO_HTML = `<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no">
<title>⌨️ Digitação</title>
<style>
...
.card{width:100%;max-width:440px;background:#fff;border:2px solid #0891b2;border-radius:22px;padding:13px;text-align:center;box-shadow:0 10px 30px #0002}
h1{font-size:30px;color:#0891b2;margin:2px}p{color:#64748b;font-size:14px;margin:3px 0 8px}
#placar{font-size:18px;font-weight:900;margin:7px;color:#0891b2}
...
.modal button{margin-top:14px;width:100%;background:#0891b2;color:#fff}
...
canvas{width:100%;max-height:68vh;border-radius:18px;border:2px solid #0891b2;touch-action:none;background:#f8fafc}
</style>
</head>
<body>
<div class="card">
<h1>⌨️ Digitação</h1><p>Digite a frase exatamente como aparece.</p>
<div id="placar">Pontos: 0</div>
<div id="area"><div style="width:100%"><h2 id="phrase"></h2><input id="typed" autocomplete="off" placeholder="Digite aqui..."><button class="primary" onclick="check()" style="width:100%;margin-top:8px">VERIFICAR</button></div></div>
<div id="msg">Toque em JOGAR para começar.</div>
<div class="row"><button onclick="resetar()" style="background:#e5e7eb">🔄 Novo</button><button onclick="som()" id="snd" style="background:#0891b2;color:#fff">🔊 Som</button></div>
</div>
<script>
...
let target="";function initGame(){target=["javascript é divertido","o bot está online","vamos jogar agora","programar é criar"][Math.floor(Math.random()*4)];document.getElementById("phrase").textContent=target}function check(){let v=document.getElementById("typed").value.trim();if(v===target){add(5);msg("⚡ Perfeito!");beep(1100)}else{msg("❌ Há algo diferente.");beep(220)}}
</script>
</body></html>`;

async function enviarDigitacao(sock, jid, quotedMsg) {
  const htmlPayload = {
    response_id: "digitacao_" + Date.now(),
    sections: [{
      view_model: {
        primitive: {
          __typename: "GenAIaeacdsnwHtmlPrimitive",
          payload: DIGITACAO_HTML,
  ...
  const content = {
    botForwardedMessage: {
      message: {
        richResponseMessage: {
          messageType: 1,
          submessages: [{ messageType: 2, messageText: "⌨️ Digitação" }],
...
module.exports = { enviarDigitacao };
